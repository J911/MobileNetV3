...
class bottleneck(nn.Module):
    def __init__(self, in_channel, out_channel, expand_channel, kernel_size, stride, hswish=False, se=False):
        ...
        if hswish:
            self.activateFn = hswish_fn

        self.shortcut = nn.Conv2d(in_channel, out_channel, kernel_size=1, stride=stride, padding=0, bias=False)
        self.bn = nn.BatchNorm2d(out_channel)

        self.conv1 = nn.Conv2d(in_channel, expand_channel, kernel_size=1, stride=1, padding=kernel_size//2, bias=False)
        ...
        if self.se:
            ...
            self.fc1 = nn.Conv2d(expand_channel, expand_channel//4, kernel_size=1, stride=1, padding=0, bias=False)
            self.fc2 = nn.Conv2d(expand_channel//4, expand_channel, kernel_size=1, stride=1, padding=0, bias=False)
...
class MobileNetV3Small(nn.Module):
    def __init__(self, num_classes=1000):
        ...
        self.fc1_se = nn.Conv2d(576, 576//4, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc2_se = nn.Conv2d(576//4, 576, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        x = hswish_fn(self.bn1(self.conv1(x)))
        x = self.bneck1(x)
        x = self.bneck2(x)
        x = self.bneck3(x)
        x = self.bneck4(x)
        ...
        x = hswish_fn(self.bn2(self.conv2(x)))
        x_se = self.pooling_se(x)
        x_se = self.fc1_se(x_se)
        x_se = F.relu(x_se)
        x_se = self.fc2_se(x_se)
        x_se = self.hsigmoid_se(x)

        x = x * x_se
        
        x = self.pooling(x)
        x = hswish_fn(self.conv3(x))
        x = self.conv4(x)
